...
class Schema {
  constructor(obj = {}, opts = {}) {
    ...
    ObjectUtil.keys(obj).forEach(k => this.path(k, obj[k]));
  }
  ...
  path(path, rules) {
    ...
    ObjectUtil.keys(rules).forEach(key => {
      let rule = rules[key];
  ...
  typecast(obj, props = this.generateProps(obj)) {
    ...
    ObjectUtil.keys(props).forEach(key => {
  ...
  strip(obj, props = this.generateProps(obj), prefix) {
    ObjectUtil.keys(obj).forEach(key => {
  ...
  validate(obj, opts = {}) {
    ...
    ObjectUtil.keys(props).forEach(path => {
  ...
  generateProps(obj) {
    ...
    ObjectUtil.keys(this.props).forEach(key => {
      ...
      ObjectUtil.keys(paths).forEach(p => props[p] = this.props[paths[p]]);
    })
  ...
  message(name, message) {
    ...
    ObjectUtil.keys(name).forEach(k => this.messages[k] = name[k]);
    return this;
  }
  ...
  validator(name, fn) {
    ...
    ObjectUtil.keys(name).forEach(k => this.validators[k] = name[k]);
    return this;
  }
